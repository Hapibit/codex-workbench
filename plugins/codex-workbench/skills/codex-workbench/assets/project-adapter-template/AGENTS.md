...
5. 先按 `FEATURE_WORKFLOW.md` 的状态机判断 `light`、`standard` 或 `strict`；先查 hard triggers，再按影响范围、不确定性、回滚难度判断，不要凭感觉降级。
6. 对 `standard` / `strict` 改动，按 `FEATURE_WORKFLOW.md` 建立功能工作包，先完成 `CHANGE_REQUEST.md`、`IMPACT_ANALYSIS.md`、`SPEC.md`、`DESIGN.md`、`PLAN.md`、`TASKS.md`，再实现。
...
如果 AI 被用户指出“跳过工作台规则、推进错范围、没有按阶段走、没有搜索却声称参考资料”，必须进入偏离复盘：说明偏离了哪条规则、为什么没有被现有门禁拦住、证据放在哪里、是否需要升级模板、质量门、hook、CI 或回归样例。
...
不要把 `docs/`、临时材料或 AI 自己发明的“阅读层/交付层”放进 `workbench/`。如果仓库根目录已有 `docs/`，它默认属于项目普通文档，不自动成为工作台标准阶段；引用前必须说明来源、用途和是否与工作台文件重复。
...
| 功能开发 | `workbench/features/<feature-name>/` | 缺 `CHANGE_REQUEST.md`、缺 `IMPACT_ANALYSIS.md`，或风险等级与验证计划不匹配。 |
